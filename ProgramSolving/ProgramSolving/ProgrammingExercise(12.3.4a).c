...
//this program gets 2 array day, month, day from user. And return the recent days structure
// Ex) the recent date is returned
...
// this function gets 2 array structure and judge what is recent date between 2 array data and return recent date array. 
// It is call by value function
struct Date recent(struct Date d[])	
{						
    ...
    else if (d[0].year==d[1].year) // when year is same, compare the month
    {
        ...
        {
            if (d[0].day < d[1].day) // when year and month is same, compare the days.
            {
                return d[1];
            }
            else
